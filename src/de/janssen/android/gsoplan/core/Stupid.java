...
public class Stupid {
	...
	public final String[] timeslots = new String[]{"","7.45 - 8.30","8.30 - 9.15","9.35 - 10.20","10.20 - 11.05","11.25 - 12.10","12.10 - 12.55","13.15 - 14.00","14.00 - 14.45","15.05 - 15.50","15.50 - 16.35","16.55 - 17.40","17.40 - 18.25","18.25 - 19.10","19.30 - 20.15","20.15 - 21.00"};
	...
	/*
	 * @author Tobias Janssen
	 * Konvertiert ein XmlTag zu einem mehrdimensionalen Array und steuert dabei einen progress dialog an
	 * 
	 * @param htmlTableTag		XmlTag, aus dem das Array erstellt werden soll
	 * @param pd				ProgressDialog, der Angesteuert werden soll
	...
	/*
	 * @author Tobias Janssen
	 * Konvertiert ein XmlTag zu einem mehrdimensionalen Array 
	 * 
	 * @param htmlTableTag		XmlTag, aus dem das Array erstellt werden soll
	 * @return					WeekData ergebnis, des XmlTag
	 */
	...
	public void fetchSelectorsFromNet(MyContext ctxt) throws Exception
	{
		...
		try
        {
        	...
        	Xml xml = new Xml();
        	xml.container = Xml.readFromURL(url,ctxt.progressDialog,Const.CONNECTIONTIMEOUT);
	...
	/*
	 * @author Tobias Janssen
	 * Liest die Elemente aus dem Xml, und fügt diese dem Stupid-Objekt hinzu
	 * @param xml			Xml das die Elemente enthält
	 * @param ctxt			MyContext der Applikation
	 ...
	 * @throws Exception					Wenn xml üngültig ist
	 */
	public DownloadFeedback fetchTimeTableFromNet(String selectedStringDate, String selectedElement, String selectedType,MyContext ctxt) throws Exception
	{
		int dataIndex = -1;
		...
		try
        {
			...
			Xml xml = new Xml();
			xml.container = Xml.readFromURL(url,ctxt.progressDialog,Const.CONNECTIONTIMEOUT);
        	xmlArray = Xml.xmlToArray(xml,ctxt.progressDialog);
        }
		catch(Exception e)
        {
        	throw e;
        ...
        WeekData weekData = new WeekData(this);
        weekData = convertXmlTableToWeekData(xmlTimeTable,ctxt.progressDialog);		//Konvertiert den angegebenen XmlTable in das WeekData-format.
	 ...
	 * @return 			ein String, der durch 0 und 1 angibt, ob das element an dieser Position den vorherigen gleicht
	 */
	 ...
	 * @param array		SelectOptions-Array in dem nach dem Wert gesucht werden soll
	 * @param value		String nach dem gesucht werden soll
	 */
	...
	/*
	 * @author Tobias Janssen
	 * Sucht den Index im Pager für das angegebene Datum heraus
	 * @param aquiredDate		Calendar der das zu suchende Datum enthält
	 * 
	 * @return 			Liefert den Index der WeekData des angegebenen Datums<p>Wenn nicht vorhanden(also bereits im Speicher geladen), wird -1 zurückgeliefert
	 */
	...
	/*
	 * @author Tobias Janssen
	 * Sucht in dem WeekData den letzten freien Platz
    ...
    public void checkAvailibilityOfWeek(MyContext ctxt, Boolean forceRefresh, int weekOffset)
    {
        ...
        if(ctxt.weekDataIndexToShow !=-1)
        {
        	...
        	{
	            Date date = new Date();
	        	if(ctxt.stupid.myTimetables[ctxt.weekDataIndexToShow].syncTime + (ctxt.stupid.myResync*60*1000) < date.getTime() || forceRefresh)	
	        	{
